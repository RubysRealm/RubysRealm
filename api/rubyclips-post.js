...
function validateManifest(m, tag) {
  ...
  if (!Number.isInteger(index) || !Number.isInteger(total) || index < 1 || total < 1 || index > total) {
    throw new Error('Blocked: invalid Facebook part numbering.');
  }
  if (!(segmentSeconds > 0 && segmentSeconds <= MAX_AUTO_SECONDS)) {
    throw new Error('Blocked: Facebook part is outside automatic TikTok duration limits.');
  }

  // A source is only divided when its original duration exceeds the automatic
  // TikTok posting limit. The footage itself remains the existing Facebook video.
  if (total > 1) {
    if (!(sourceSeconds > MAX_AUTO_SECONDS) || m?.technicalSplitOnly !== true) {
      throw new Error('Blocked: unverified Facebook split.');
    }
    if (m?.titleBurnedIn !== true || m?.partLabelBurnedIn !== true) {
      throw new Error('Blocked: multipart title/part layout is missing.');
    }
    if (tag !== `rubyclips-fb-${m.sourceVideoId}-s${index}`) {
      throw new Error('Blocked: release tag does not match Facebook part.');
    }
  } else {
    if (m?.technicalSplitOnly === true) throw new Error('Blocked: unexpected Facebook split flag.');
    if (tag !== `rubyclips-fb-${m.sourceVideoId}`) {
      throw new Error('Blocked: release tag does not match Facebook video.');
    }
  }
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      videoUrl,
      renderer: 'facebook-existing-video-parts-v4'
    });
  } catch (error) {
