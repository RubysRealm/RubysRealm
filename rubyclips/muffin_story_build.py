...
OUTPUT_FPS = 30
PIPELINE_REVISION = 'avsync-v4-direct-builder'
CONTENT_WIDTH = 900
CONTENT_HEIGHT = 1600
...
def esc_filter_value(value):
    return str(value).replace('\\','\\\\').replace(':','\\:').replace("'","\\'")


FONT_FILTER = esc_filter_value(FONT)

# Build title-card preview with a solid black frame around the complete source art.
...
if cover_candidates:
    ...
    cover_filter = (
        ...
        f"[coverbase]drawtext=fontfile='{FONT_FILTER}':textfile='{esc_path(WORK/'story-title.txt')}':"
        "fontcolor=white:fontsize=58:line_spacing=8:box=1:boxcolor=black@0.72:boxborderw=20:"
        "x=(w-text_w)/2:y=500,"
        f"drawtext=fontfile='{FONT_FILTER}':textfile='{esc_path(WORK/'part-label.txt')}':"
...
filters.append(
    f"[vcat]drawtext=fontfile='{FONT_FILTER}':textfile='{esc_path(WORK/'story-title.txt')}':"
...
manifest = {
    ...
    'sourceFrameMode': 'solid-black-pillarbox-v2',
