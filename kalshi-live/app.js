...
async function verifyHistory(currentTicker){
  ...
  const results=await Promise.all(todo.map(async row=>{
    try{
      const r=await fetch('/kalshi-api/api/verify?ticker='+encodeURIComponent(row.contract_id)+'&ts='+Date.now(),{cache:'no-store'});
...
async function tick(force=false){
  ...
  busy=true;
  try{
    const r=await fetch('/kalshi-api/api/state?ts='+Date.now(),{cache:'no-store'}),s=await r.json();
...
async function watchSession(){
  try{
    const r=await fetch('/kalshi-api/api/session?ts='+Date.now(),{cache:'no-store'}),j=await r.json(),m=j.market;
...
function connectPrice(){
  try{
    ...
    priceStream=new EventSource('/kalshi-api/api/price-stream');
