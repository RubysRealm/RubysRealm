...
function status(extra = {}) {
  ...
  const merged = {
    stage: 'waiting_for_login', signedIn: false, downloading: false,
    ...
    readyParts: readyParts(), updatedAt: new Date().toISOString(),
    ...prior, ...extra, readyParts: readyParts(), updatedAt: new Date().toISOString()
  };
  fs.writeFileSync(STATUS, JSON.stringify(merged, null, 2));
  return merged;
}
...
function probeLogin() {
  ...
  const ok = r.status === 0 && String(r.stdout || '').trim().split(/\s+/).includes(VIDEO_ID);
  if (!ok && r.stderr) console.log('yt-dlp login probe:', String(r.stderr).slice(-700));
  return ok;
}

async function acquire() {
  status({ stage: 'login_confirmed', signedIn: true, downloading: true, error: null });
  await run(YTDLP, [
    '--ffmpeg-location', ffmpegPath,
    '--cookies-from-browser', `chrome+basictext:${PROFILE}`,
    '--no-playlist', '--retries', '20', '--fragment-retries', '20', '--concurrent-fragments', '4',
  ...
  status({ stage: 'splitting_source', signedIn: true, downloading: true });
  for (let part = 1; part <= TOTAL_PARTS; part++) {
    ...
    status({ stage: `part_${part}_ready`, signedIn: true, downloading: true });
  }
  ...
  status({ stage: 'ready', signedIn: true, downloading: false, error: null });
}

status({ stage: 'starting_browser', signedIn: false, downloading: false, error: null });
const context = await chromium.launchPersistentContext(PROFILE, {
  headless: true,
  executablePath: CHROME,
  viewport: { width: 720, height: 1280 },
  args: [
    '--no-sandbox','--disable-dev-shm-usage','--password-store=basic','--no-first-run',
    '--no-default-browser-check','--disable-features=TranslateUI','--disable-blink-features=AutomationControlled'
  ]
});
let page = context.pages()[0] || await context.newPage();
await page.goto(VIDEO_URL, { waitUntil: 'domcontentloaded', timeout: 120000 }).catch(e => console.log('initial navigation:', e.message));
status({ stage: 'waiting_for_login', signedIn: false, downloading: false });

let acquisitionStarted = false;
setInterval(async () => {
  if (acquisitionStarted || readStatus().stage === 'ready') return;
  if (!probeLogin()) return;
  acquisitionStarted = true;
  try { await acquire(); }
  catch (e) {
    console.error(e);
    status({ stage: 'error', signedIn: true, downloading: false, error: String(e?.message || e) });
...
app.get('/', (req,res) => { const s=readStatus(); res.type('html').send(`<meta name="viewport" content="width=device-width,initial-scale=1"><body style="background:#090a0f;color:#fff;font:18px system-ui;padding:28px"><h1>Rubradaclips YouTube Source</h1><p>Status: <b>${s.stage}</b></p><p>Ready parts: ${(s.readyParts||[]).length}/${s.totalParts}</p></body>`); });
...
app.get('/api/desktop-frame', auth, async (req,res) => {
  try {
    ...
    res.setHeader('Content-Type','image/jpeg'); res.setHeader('Cache-Control','no-store'); res.end(buf);
  } catch(e) { res.status(503).send(String(e.message)); }
});
app.post('/api/desktop-click', auth, async (req,res) => {
  try { await page.mouse.click(Math.max(0,Math.min(719,Number(req.body?.x)||0)), Math.max(0,Math.min(1279,Number(req.body?.y)||0))); res.json({ok:true}); }
  catch(e){res.status(500).json({ok:false,error:e.message});}
});
app.post('/api/desktop-type', auth, async (req,res) => {
  try { await page.keyboard.type(String(req.body?.text||'').slice(0,500), {delay:20}); res.json({ok:true}); }
  catch(e){res.status(500).json({ok:false,error:e.message});}
});
app.post('/api/desktop-key', auth, async (req,res) => {
  const map={Return:'Enter',Tab:'Tab',Escape:'Escape',BackSpace:'Backspace',Up:'ArrowUp',Down:'ArrowDown',Left:'ArrowLeft',Right:'ArrowRight',space:'Space','ctrl+l':'Control+L'};
  const key=map[String(req.body?.key||'')]; if(!key)return res.status(400).json({ok:false});
  try { await page.keyboard.press(key); res.json({ok:true}); } catch(e){res.status(500).json({ok:false,error:e.message});}
});
...
app.get('/source/:name', (req,res) => {
  ...
  const f=path.join(SOURCE_DIR,name); if(!fs.existsSync(f))return res.status(404).end();
