...
RUNTIME_BRANCH=takarada-virtual-phone
SCREEN_PATH=virtual-phone/runtime/screen.enc
...
# Install the exact Takarada display APK built by GitHub Actions.
curl -fL --retry 3 \
...
publish_binary() {
  ...
  sha=$(gh api "repos/$GITHUB_REPOSITORY/contents/$path?ref=$RUNTIME_BRANCH" --jq '.sha' 2>/dev/null || true)
  if [ -n "$sha" ]; then
    gh api --method PUT "repos/$GITHUB_REPOSITORY/contents/$path" \
      -f message="$message" -f content="$content" -f branch="$RUNTIME_BRANCH" -f sha="$sha" >/dev/null
  else
    gh api --method PUT "repos/$GITHUB_REPOSITORY/contents/$path" \
      -f message="$message" -f content="$content" -f branch="$RUNTIME_BRANCH" >/dev/null
  fi
}
...
PY
  ...
  publish_binary "$SCREEN_PATH" "$ROOT/screen.enc" "Takarada phone screen $seq"
  post_comment "TAKARADA_SCREEN_READY|$seq|$iv|$SCREEN_PATH"
}
...
# Keep Aurora available as a fallback installer; failure does not stop control.
(
...
while [ $SECONDS -lt $END ]; do
  ...
  if [[ "$BODY" == TAKARADA_CMD\|* ]]; then
    ...
    if [[ "$seq" =~ ^[0-9]+$ ]] && [ "$seq" -gt "$LAST_SEQ" ]; then
      ...
      LAST_SEQ=$seq
      sleep 2
      publish_screen "$seq" || post_comment "TAKARADA_AGENT_ERROR|$seq|screenshot_failed"
    fi
  fi
  sleep 2
done
