...
function render(s,row){
  ...
  el('selentry').textContent=row&&row.entry_price!=null?Math.round(Number(row.entry_price)*100)+'¢':'—';if(row&&row.skip_late){
    el('pick').textContent='WAIT NEXT ROUND';el('pick').className='pick pending';
    el('conf').textContent='Kalshi test opened after the profitable entry window • no late entry will be simulated';
    ...
    el('status').textContent='Kalshi test analysis window • snapshots at 30 / 60 / 90 / 115 sec • $1 sim only enters at 67¢ or cheaper';
  }else{
...
async function watchSession(){
  try{
    ...
    if(seenContract&&m.contract_id!==seenContract){
      ...
      el('pick').textContent='ANALYZING';el('pick').className='pick';
      el('conf').textContent='New round • Kalshi test analyzing for about 115 seconds before lock';
      tick(true);
