...
def load_queue():
    # Rebuild every run so a newly supplied reference/source policy immediately takes effect.
    source_queue.build_queue()
    payload = json.loads(QUEUE_PATH.read_text())
    candidates = payload.get("candidates") or []
    if not candidates:
        raise RuntimeError("Halo-style source queue is empty.")
    return payload, candidates


def candidate_is_allowed(candidate):
    if candidate.get("niche") != "gaming":
        return False
    license_kind = str(candidate.get("license_kind") or "").lower()
    if license_kind not in ("public-domain", "cc-by", "cc-by-3.0", "cc-by-4.0", "gpl-3.0"):
        return False
    # This lane is intentionally strict: the user supplied ShinRegis Halo reels as
    # the reference and explicitly rejected unrelated generic gaming footage.
    style = str(candidate.get("style") or "").lower()
    return style.startswith("halo-")
...
def download_candidate(candidate):
    ...
    req = urllib.request.Request(
        url,
        headers={"User-Agent": "RubysRealmFacebookSource/1.0 (sourced reusable media)"},
    )
...
def source_quality_ok(path, info):
    ...
    if path.stat().st_size < 10000:
        return False, "source file too small"
    return True, "ok"
...
def select_source(state):
    ...
    if current and current.get("candidate"):
        ...
        if not candidate_is_allowed(candidate):
            # Drop the old generic-gameplay lock immediately. Do not continue a
            # source that no longer matches the user's Halo reference.
            state["current"] = None
    ...
    for queued in queue[:MAX_CANDIDATE_TRIES]:
        ...
        try:
            ...
            if not candidate or not candidate_is_allowed(candidate):
                errors.append(f"{queued.get('key')}: rejected by Halo source policy")
                continue
            source = download_candidate(candidate)
            info = base.probe(source)
            ok, reason = source_quality_ok(source, info)
    ...
    raise RuntimeError(
        "No suitable reusable Halo-style sourced candidate passed ingest checks. "
        + "; ".join(errors[-6:])
    )
...
def render_clean(source, part_index, segments, creator):
    start, end = segments[part_index - 1]
    duration = max(1.0, end - start)
    out = OUT / f"facebook-part-{part_index:02d}-of-{len(segments):02d}.mp4"
    credit = ffmpeg_text(f"Source: {creator}" if creator else "")
    base_vf = (
        "[0:v]split=2[bg0][fg0];"
        "[bg0]scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920,gblur=sigma=34[bg];"
    ...
    if credit:
        base_vf += (
            ...
            f"text='{credit}':x=38:y=42:fontsize=30:fontcolor=white:"
            "box=1:boxcolor=black@0.55:boxborderw=12:shadowx=2:shadowy=2[v]"
        )
    else:
        base_vf += "[v]"
    run([
...
def post_description(candidate, part, total):
    ...
    lines = ["Halo moments are undefeated 😂"]
    if creator:
        lines += ["", f"Source: {creator}"]
    if total > 1:
        lines += ["", f"Part {part} of {total}"]
    lines += ["", "#halo #gaming #machinima #funny #reels"]
    return "\n".join(lines).strip()
...
def main():
    ...
    creator = base.safe_text(candidate.get("creator"), "")
    video, out_info, start, end = render_clean(source, part_index, segments, creator)
    try:
    ...
    evidence = {
        ...
        "referenceCreator": candidate.get("reference_creator"),
        "referenceUrl": candidate.get("reference_url"),
    ...
    title = base.safe_text(candidate.get("title") or candidate.get("title_hint"), "Sourced Halo video")
    manifest = {
        "pipeline": "rubysrealm-sourced-halo-style-facebook-v5",
        ...
        "referenceStyleCreator": candidate.get("reference_creator"),
        "sourceCreditShownInVideo": bool(creator),
        "sourceCreditShownInDescription": bool(creator),
    }
