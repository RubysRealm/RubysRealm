...
export async function findBufferVideoPostBySource({ channelId, videoUrl, allowDisabled = false, first = 60 }) {
  ...
  return posts.find(post => post?.assets?.some(asset => asset?.source === videoUrl)) || null;
}

export async function createBufferVideoPost({ channelId, caption, videoUrl, dueAt = null, allowDisabled = false, dedupeVideoUrl = false }) {
  ...
  if (dedupeVideoUrl) {
    const duplicate = await findBufferVideoPostBySource({ channelId, videoUrl, allowDisabled });
