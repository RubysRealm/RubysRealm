...
USED=set()
PROFILE={'name':'story','anchors':{'building','office','room'},'queries':['building interior','building exterior'],'banned':set()}

PROFILES=[
 (('marina',), {'name':'marina','anchors':{'marina','dock','harbor','boat','yacht','pier','slip'},'queries':['marina boats docks','harbor marina boats','marina pier'],'banned':{'saint','church','mosaic','carta','hotel','sands'}}),
 (('drive-in','drive in'), {'name':'drive-in theater','anchors':{'drive-in','theater','cinema','screen','movie'},'queries':['drive-in movie theater','drive-in theater screen','outdoor cinema screen'],'banned':{'driveway','residence','house','street'}}),
 (('diner',), {'name':'diner','anchors':{'diner','restaurant','counter','cafe','booth'},'queries':['american diner interior','roadside diner exterior','diner counter booths'],'banned':set()}),
 (('mountain lodge','lodge'), {'name':'mountain lodge','anchors':{'lodge','cabin','hotel','resort','mountain'},'queries':['mountain lodge exterior','mountain lodge interior','rustic lodge cabin'],'banned':{'dance','tribal'}}),
 (('ferry',), {'name':'ferry terminal','anchors':{'ferry','terminal','dock','boat','pier'},'queries':['ferry terminal dock','ferry boat terminal','river ferry dock'],'banned':set()}),
 (('warehouse',), {'name':'warehouse','anchors':{'warehouse','industrial','factory','storage'},'queries':['industrial warehouse interior','warehouse exterior','warehouse loading bay'],'banned':set()}),
 (('radio station',), {'name':'radio station','anchors':{'radio','studio','broadcast','microphone','transmitter'},'queries':['radio station studio','broadcast radio studio','radio transmitter building'],'banned':set()}),
 (('greenhouse',), {'name':'greenhouse','anchors':{'greenhouse','nursery','plants','horticulture'},'queries':['commercial greenhouse interior','greenhouse plants','plant nursery greenhouse'],'banned':set()}),
 (('storage',), {'name':'storage facility','anchors':{'storage','warehouse','locker','unit'},'queries':['self storage facility','storage unit interior','self storage corridor'],'banned':set()}),
 (('theater',), {'name':'movie theater','anchors':{'theater','cinema','movie','screen','auditorium'},'queries':['old movie theater interior','cinema auditorium','movie theater projection booth'],'banned':set()}),
]
...
INTENTS=[
 ...
 ('key', ('key rack','key tag','brass key','keys','key'), {'key','keys','padlock','lock'}, ['brass key closeup','vintage keys','key rack']),
 ...
 ('truck', ('pickup truck','box truck','tow truck','utility van','sedan','suv','vehicle'), {'truck','pickup','van','sedan','suv','vehicle','car'}, ['pickup truck roadside','utility van parking lot','sedan parking lot']),
 ('phone', ('desk phone','telephone','intercom','phone'), {'telephone','phone','intercom'}, ['old desk telephone','office telephone','intercom speaker']),
...
STOP={'your','life','owner','manager','buyer','after','buying','closed','old','current','shown','several','before','after','years','someone','simple','answer','properties','always','finally','recent','entries','company','business'}
NEGATIVE_GLOBAL={'logo','icon','diagram only','clipart','cartoon','flag','coat of arms'}


def set_context(title):
 global PROFILE, USED
 low=str(title).lower()
 PROFILE={'name':'story','anchors':set(), 'queries':['building interior','building exterior'],'banned':set()}
 for needles,p in PROFILES:
  if any(n in low for n in needles):
   PROFILE={'name':p['name'],'anchors':set(p['anchors']),'queries':list(p['queries']),'banned':set(p['banned'])}
   break
...
def _score(query,c):
 ...
 if PROFILE['banned'] and any(re.search(r'\b'+re.escape(x)+r'\b',text) for x in PROFILE['banned']): return -999
 ...
 if PROFILE['name']=='drive-in theater' and not any(x in text for x in ('drive-in','drive in','theater','cinema','movie','screen')): return -999
 if name and not any(re.search(r'\b'+re.escape(x)+r'\b',text) for x in positive): return -999
 overlap=sum(1 for t in qtokens if len(t)>3 and re.search(r'\b'+re.escape(t)+r'\b',text))
 context=sum(1 for t in PROFILE['anchors'] if re.search(r'\b'+re.escape(t)+r'\b',text))
 intent_hits=sum(1 for t in positive if re.search(r'\b'+re.escape(t)+r'\b',text))
 score=overlap*5+context*5+intent_hits*10
 if name and intent_hits>=1: score+=8
 if not name and context==0: score-=8
...
def fetch_photo(query,seed,dest):
 ...
 ranked=sorted(uniq.values(),key=lambda c:(str(c.get('url')) in USED,-int(c.get('_semantic_score',0)),-min(int(c.get('width') or 0),int(c.get('height') or 0))))
 if not ranked:
  return {'query':q,'source_type':'none','error':'No semantically adequate public-domain/CC0 photo found'}
 unused=[c for c in ranked if str(c.get('url')) not in USED]
 choices=unused or ranked
 pick=choices[int(seed)%min(5,len(choices))]
 raw=Path(dest).with_suffix('.download')
 try:
  base.download(pick['url'],raw)
  with Image.open(raw) as im:
   im=ImageOps.exif_transpose(im).convert('RGB')
   if im.width<700 or im.height<430: raise RuntimeError('image too small')
  ...
  raw.unlink(missing_ok=True)
  return {'query':q,'source_type':'none','error':str(e)[:300]}
