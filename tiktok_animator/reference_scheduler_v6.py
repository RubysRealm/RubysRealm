def select_visuals(semantic, beats, duration):
    ...
    while i < len(ordered):
        if not selected:
            ...
            if window:
                ...
                future=[x for x in ordered if float(x['start'])>last+8.5]
                if not future: break
                before=[x for x in future if float(x['start'])<=last+18.3]
                b=max(before,key=lambda x:(float(x.get('score',0)),-abs(float(x['start'])-target))) if before else future[0]
        start=float(b['start'])
        if selected and start-last<8.5:
            i=ordered.index(b)+1
            continue
        available=max(0.0,float(duration)-start-0.15)
        hold=min(6.8,max(5.4,float(b['end'])-start+0.6),available)
        if hold>=3.2:
    ...
    for _ in range(8):
        ...
        candidates=[b for b in ordered if float(a['start'])+8.5<=float(b['start'])<=float(z['start'])-8.5]
        if not candidates: break
        b=max(candidates,key=lambda x:(float(x.get('score',0))*1.5-abs(float(x['start'])-target)))
        st=float(b['start']); hold=min(6.4,max(5.2,float(b['end'])-st+0.5),max(0.0,duration-st-0.15))
        if hold<3.2: break
    ...
    clean=[]
    for v in selected:
        if clean and float(v['start'])-float(clean[-1]['start'])<8.4:
