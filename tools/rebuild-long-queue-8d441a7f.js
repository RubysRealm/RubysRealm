import autoPost from '../api/auto-post.js';
...
async function gql(query, variables = {}) {
  ...
  for (let attempt = 0; attempt < 7; attempt++) {
    ...
    if (response.status === 429) {
      ...
      const waitMs = Number.isFinite(retryAfter) && retryAfter > 0 ? Math.min(retryAfter * 1000, 60000) : Math.min(2500 * (2 ** attempt), 45000);
...
async function scheduledPosts(found) {
  const data = await gql(`query Scheduled($organizationId: OrganizationId!, $channelId: ChannelId!) { posts(first: 20, input: { organizationId: $organizationId, filter: { status: [scheduled], channelIds: [$channelId] }, sort: [{ field: dueAt, direction: asc }] }) { edges { node { id text dueAt } } } }`, { organizationId:found.organization.id, channelId:found.channel.id });
  return (data?.posts?.edges || []).map(edge => edge.node);
}

async function deletePost(id) {
  const data = await gql(`mutation DeleteScheduled($id: PostId!) { deletePost(input: { id: $id }) { ... on DeletePostSuccess { id } ... on VoidMutationError { message } } }`, { id });
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (mode === 'clear' || mode === 'rebuild') {
      ...
      const deleted = [];
      for (const post of posts) { deleted.push(await deletePost(post.id)); await sleep(1200); }
