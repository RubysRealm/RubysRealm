...
function renderStats(){
  ...
  }else el('learndetail').textContent='Learning only from strictly verified V2 rounds.';
...
function render(s,row){
  ...
  if(row&&row.late_test){el('valuestate').textContent='TEST ONLY — not counted';
    ...
    el('status').textContent='This pick is saved and will be graded from the official settlement. Next round returns to the normal 55-second lock.';
  }else if(!row){
...
async function tick(force=false){
  ...
  try{
    ...
    if(s.maintenance||!s.market){
      const cp=Number.isFinite(Number(s.spot?.coinbase_app_spot))?Number(s.spot.coinbase_app_spot):Number(s.spot?.exchange_price);
      const bp=Number(s.spot?.price);
      ...
      el('price').textContent=money(cp);el('ptb').textContent='—';el('dist').textContent='No active round';
      ...
      el('cbmeta').textContent='Coinbase Spot';el('cbcompare').textContent='BRTI proxy '+money(bp)+' • Coinbase '+money(cp);
