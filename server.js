...
function state(){return {index,count:videos.length,playing,current:cur(),next:videos[(index+1)%videos.length],broadcasting:!!ffmpeg,tiktokConnected,username:TIKTOK_USERNAME||null,chat:recentChat.slice(-8),events:recentEvents.slice(-8)}}
...
function addEvent(type,text,extra={}){const e={type,text,ts:Date.now(),...extra};recentEvents.push(e);recentEvents=recentEvents.slice(-50);io.emit("event",e)}
...
setInterval(()=>addEvent("commentary",commentary(cur().title)),120000+Math.floor(Math.random()*90000));
