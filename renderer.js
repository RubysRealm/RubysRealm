...
async function playerSnapshot(page,id){return await page.evaluate((videoId)=>{const v=document.querySelector('video'),p=document.getElementById('movie_player');if(!v)return{code:-1,label:'loading',error:'YouTube watch video element not ready',videoId,position:0};const ad=!!p?.classList.contains('ad-showing');let code,label;if(v.error){code=-1;label='error'}else if(v.ended){code=0;label='ended'}else if(v.paused){code=2;label='paused'}else if(v.readyState<2){code=3;label='buffering'}else{code=1;label='playing'}return{code,label,error:v.error?.message||null,videoId,position:Number(v.currentTime||0),duration:Number.isFinite(v.duration)?Number(v.duration):0,volume:v.volume,muted:v.muted,ad};},id).catch(e=>({code:-1,label:'error',error:e.message,videoId:id,position:0}));}
while(!stopping){
 try{
  ...
  const page=await context.newPage();page.on('pageerror',e=>console.log('Renderer page error:',e.message));let currentId=null,endedId=null,lastLabel=null;console.log('Cloud renderer: gameplay-only YouTube compositor');
  while(!stopping&&!page.isClosed()){
   const st=await getState(),wanted=st.current?.id;if(wanted&&wanted!==currentId){currentId=wanted;endedId=null;await playWatch(page,currentId);}await installLayout(page);await skipAd(page);let s=await playerSnapshot(page,currentId);
   if(s.label==='paused'&&!s.ad){await page.evaluate(()=>{const v=document.querySelector('video');if(v){v.muted=false;v.volume=1;v.play().catch(()=>{});}}).catch(()=>{});await pause(250);s=await playerSnapshot(page,currentId);}
   if(s.label!==lastLabel){console.log('Cloud playback:',JSON.stringify(s));lastLabel=s.label;}await post('/api/player-state',{...s,renderer:'cloud'});if(s.label==='ended'&&s.videoId&&s.videoId!==endedId){endedId=s.videoId;await post('/api/video-ended',{videoId:s.videoId});}if(s.label==='playing')endedId=null;
 ...
 }catch(e){console.log('Cloud renderer stopped:',e.message);await post('/api/player-state',{renderer:'cloud',code:-995,label:'error',error:e.message}).catch(()=>{});}await browser?.close().catch(()=>{});if(!stopping)await pause(8000);
}
